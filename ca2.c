...
void main(int argc, char* argv[])
{
	...
	ca2_I_1_input = (float **) malloc(image_size * sizeof(float *));
	ca2_I_1_output = (float **) malloc(image_size * sizeof(float *));
	...
	for (i = 0; i < image_size; i++) 
	{
		...
		ca2_I_1_input[i] = (float *) malloc(image_size * sizeof(float));
		ca2_I_1_output[i] = (float *) malloc(image_size * sizeof(float));
	}
    ...
    /* smoothing filter */
/*    image_float_temp = image_float;
    for(row = 1; row < image_size-1; row++)
	{
		for(column = 1; column < image_size-1; column++) 
		{
			image_float[row][column] = 0.2*(image_float_temp[row][column]+image_float_temp[row-1][column]+image_float_temp[row+1][column]+image_float_temp[row][column-1]+image_float_temp[row][column+1]);
        }
	}
*/
	...
	/*for(row=0;row<image_size;row++)
	{
		...
		{
			ca2_I_1_input[row][column]=0;
		}
	}
	ca2_I_1_input[0][0]=1;	
	IIR_filter(ca2_I_1_input, image_float, image_size);
	for(row=0;row<image_size;row++)
	{
		for(column=0;column<image_size;column++)
		{
			if(image_float[row][column]!=0) {image_float[row][column]=255;}
		}
	}*/
	/******************************************/
	...
	IIR_filter(image_float, image_float, image_size);
	float max=maximum_value(image_float, 1, 256);
	for(row=0;row<image_size;row++)
	{
		for(column=0;column<image_size;column++)
		{
			image_float[row][column]=image_float[row][column]/max*255;
		}
	}
	/******************************************/
